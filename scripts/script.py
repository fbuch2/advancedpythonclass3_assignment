...
@click.option(
    "--output", "-o", help="Where the output will be stored", default="output"
)
@click.option("--year", "-y", type=click.INT, help="Choose year to filter by")
@click.option(
    "--genre",
    "-g",
    help="Choose genre to filter by (Adventure, Action, Drama, Comedy, Thriller or Suspense, Horror, Romantic Comedy, Musical, Documentary, Dark Comedy, Western, Concert or Performance, Multiple Genres, Reality)",
)
@click.option(
    "--gross",
    "-gr",
    type=click.INT,
    help="Choose gross amount to filter by (bigger than)",
)
@click.option(
    "--tickets_sold",
    "-t",
    type=click.INT,
    help="Choose number of tickets sold to filter by (bigger than)",
)
def main(input, year, output, genre, gross, tickets_sold):
    ...
    cols = len(df.axes[0])
    print(
        f"There is/are {cols} line/s with the desired filters. You can find the document in {output}"
    )
